...
`
	cmds := []string{
		...
		usageKeyLine("switches back to normal mode or clears marked entries", keyEsc),
		"",
		usageKeyLine("marks an entry for multiselect return", keyMark),
		usageKeyLine("marks all entries for multiselect return", keyMarkAll),
		"",
